...
class Admin(models.Model):
    ...
    role = models.CharField(max_length=20, choices=[('student', 'Student'), ('admin', 'Admin')], default="admin")
    name = models.CharField(max_length=100, default="")

    def save(self, *args, **kwargs):
        # Sync name from Student model
        if self.student and not self.name:
...
class Candidate(models.Model):
    ...
    is_active = models.BooleanField(default=True)
    position = models.CharField(max_length=50, null=True, blank=True)  # New field for position

    def __str__(self):
        return f"Candidate: {self.student.name} ({self.student.student_id}) - {self.position or 'No Position'}"
